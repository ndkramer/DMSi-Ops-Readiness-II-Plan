#!/usr/bin/env node
/**
 * WBS Load Report Counts — forwards to `dynamo-plan wbs report-counts`.
 *
 * Resolves CLI: DYNAMO_PLAN_CLI, or sibling dynamo-os/planning-toolkit/bin/cli.js.
 *
 * From project root: node Scripts/wbs-load-report-counts.js <capability> <dateStamp>
 */
...
if (!capability || !dateStamp) {
  ...
  console.error('Or: dynamo-plan wbs report-counts PA 03-17-2026');
  process.exit(1);
}
...
const result = spawnSync(
  process.execPath,
  [cli, 'wbs', 'report-counts', capability, dateStamp, '--cwd', projectRoot],
