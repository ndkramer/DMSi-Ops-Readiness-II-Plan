...
const CAPABILITY_CONFIG = {
  ...
  /** Placeholder keys — override: `node Scripts/jira-export-pa.js WB <capabilityKey> <actionEpicKey>` or use `jira-export-wb.js` after wb-outcomes.json is keyed. */
  WB: { prefix: 'WB', rootKey: 'WSA-TBD-WB-CAPABILITY', actionItemRoot: 'WSA-TBD-WB-ACTION-ITEMS' },
};
...
function parseArgs() {
  ...
  return {
    ...
    outDir: path.join(getCapabilityFolder(capability), 'Jira'),
...
/**
 * Index Sub-task issues by parent Story key for kanban checklists / progress.
 */
function indexSubtasksByParent(subtasksRaw, baseUrl) {
  const base = (baseUrl || '').replace(/\/$/, '');
  const byParent = {};
  for (const raw of subtasksRaw || []) {
    const f = raw.fields || raw;
    const typeName = (f.issuetype && f.issuetype.name) || '';
    if (!/sub-task|subtask/i.test(typeName)) continue;
    const parentKey = (f.parent && f.parent.key) || '';
    if (!parentKey) continue;
    const key = raw.key || raw.id;
    if (!key) continue;
    const st = f.status || {};
    const cat = st.statusCategory && st.statusCategory.key ? st.statusCategory.key : '';
    const item = {
      issue_key: key,
  ...
  for (const pk of Object.keys(byParent)) {
    byParent[pk].sort(function (a, b) {
      return (a.issue_key || '').localeCompare(b.issue_key || '', undefined, { numeric: true });
    });
  }
  return byParent;
}
 ...
 * - Each row includes subtasks[] when the export contained Sub-tasks under that Story.
 */
function buildKanbanJiraStatusJson(stories, baseUrl, subtasksRaw) {
  const base = (baseUrl || '').replace(/\/$/, '');
  const by_deliverable_id = {};
  const by_epic_key = {};
  const subtasksByParent = indexSubtasksByParent(subtasksRaw, baseUrl);
...
async function run() {
  ...
  // Kanban JSON/JS bundle is consumed by PA-kanban.html only; skip for other capabilities until their HTML wires a matching global.
  if (capability === 'PA') {
    const kanbanPayload = buildKanbanJiraStatusJson(stories, baseUrl, subtasks);
