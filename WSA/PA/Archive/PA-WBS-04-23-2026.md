...
- **Change log (planning ↔ Jira):**
  - **2026-04-23** — **`WSA/PA/Jira/`** removed from the repo (no committed Jira exports or `pa-kanban-jira-status.*`); export recreates the folder if run. **`PA-kanban.html`** is **WBS-embedded** only (`outcomeData` in the HTML); it does **not** load Jira sidecars. Jira remains the record for execution; update embeds when WBS changes.
  ...
  - **2026-03-24** — Removed static **POC-1…POC-4** pseudo-deliverables from the PA kanban HTML artifact; NGINX pipeline POC execution tracked in Jira with **`[POC]`** summary prefix. Kanban is driven from the Jira export (`dynamo-plan jira export` / `Scripts/legacy/jira/jira-export-pa.js` → `Jira/pa-kanban-jira-status.json`).
...
| Date | Summary |
|------|---------|
| 2026-04-23 | Removed **`WSA/PA/Jira/`** from the repo; **`PA-kanban.html`** is WBS-embedded only (no Jira sidecars). Optional export recreates `Jira/` if run. **Related:** `PA-kanban.html`, `WSA/PA/README.md`, `Documentation/legacy/Jira-*.md`. |
