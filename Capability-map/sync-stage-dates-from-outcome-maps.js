...
function parseWMOutcomes(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    const endMon = dateMatch[2].slice(0, 3);
    ...
    const year = 2026;
    outcomes[id] = { start: new Date(year, startMonthIdx, 1), end: new Date(year, endMonthIdx, 1) };
  }
  return outcomes;
}
...
function parseVIOutcomes(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    const endMonthIdx = MONTHS.indexOf(endMon);
    if (startMonthIdx === -1 || endMonthIdx === -1) continue;
    outcomes[id] = { start: new Date(2026, startMonthIdx, 1), end: new Date(2026, endMonthIdx, 1) };
  }
  return outcomes;
}
...
function computeWSBWSCStageDates(capabilityId, stageNum, activityDates) {
  ...
  return { targetStart: toMonYyyy(minStart), targetEnd: toMonYyyy(maxEnd) };
}
